...
func LoadMIDIFile(path string) ([]Note, error) {
	...
	songData := make(map[int]map[int][]MIDIEvent)
	for {
		...
		case "MTrk":
			...
			for {
				...
				case 0x90:
					...
					cd, ok := songData[int(channel)]
					if !ok {
						cd = make(map[int][]MIDIEvent)
						songData[int(channel)] = cd
					}
					...
					if velocity == 0 {
						...
						songData[int(channel)][int(noteNumber)] = append(songData[int(channel)][int(noteNumber)], MIDIEvent{
							DeltaTime: time,
							OnMessage: false,
